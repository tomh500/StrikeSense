...
float scaled(const notifications_layout::metrics& layout, float baseValue)
{
    return notifications_layout::Scale(layout, baseValue);
}
...
UINT desired_frame_ms(float elapsed, float durationMs, int style)
{
    const bool entering = elapsed < static_cast<float>(kAnimMs);
    const bool leaving = elapsed > (std::max)(0.f, durationMs - static_cast<float>(kAnimMs));
    const bool liquidbounceKnobAnimating = style == 0
        && elapsed < static_cast<float>(kLiquidBounceKnobAnimMs);
    return (entering || leaving || liquidbounceKnobAnimating) ? kFastFrameMs : kFrameMs;
}

void ensure_timer(UINT frameMs)
{
    if (!has_window()) return;
    if (s_currentTimerMs == frameMs) return;
    SetTimer(s_hwnd, kTimer, frameMs, nullptr);
    s_currentTimerMs = frameMs;
    std::cout << "[通知] 已切换动画刷新间隔: " << frameMs << "ms" << std::endl;
}
...
bool ensure_back_buffer(HDC hdcScreen, int width, int height)
{
    ...
    std::cout << "[通知] 已创建通知后备缓冲: " << width << "x" << height << std::endl;
    return true;
}
...
void draw_progress(Gdiplus::Graphics& g, const Gdiplus::RectF& box, float progress,
    const notifications_layout::metrics& layout, const Gdiplus::Color& color, bool glow)
{
    const float barHeight = scaled(layout, 4.f);
    ...
    if (glow) {
        for (int i = 3; i >= 1; --i) {
            ...
            const float glowExpand = scaled(layout, static_cast<float>(i));
            g.FillRectangle(&glowBrush, Gdiplus::RectF(back.X, back.Y - glowExpand,
                back.Width * progress, back.Height + glowExpand * 2.f));
        }
    }
...
void draw_liquidbounce(Gdiplus::Graphics& g, const Gdiplus::RectF& box,
    const notifications_layout::metrics& layout, float knobAnimProgress)
{
    fill_card(g, box, Gdiplus::Color(245, 33, 33, 33), scaled(layout, 8.f));

    const Gdiplus::RectF stateBox(box.X + scaled(layout, 14.f), box.Y + scaled(layout, 13.f),
        scaled(layout, 46.f), scaled(layout, 46.f));
    ...
    Gdiplus::Pen trackPen(Gdiplus::Color(255, 255, 255, 255), scaled(layout, 2.f));
    ...
    const float knobSize = scaled(layout, 9.f);
    ...
    Gdiplus::Font titleFont(L"Microsoft YaHei UI", scaled(layout, 15.f), Gdiplus::FontStyleBold);
    Gdiplus::Font subFont(L"Microsoft YaHei UI", scaled(layout, 10.f), Gdiplus::FontStyleRegular);
    draw_text(g, s_enabledState ? L"开启" : L"关闭", titleFont,
        box.X + scaled(layout, 74.f), box.Y + scaled(layout, 11.f),
        Gdiplus::Color(255, 255, 255, 255));
    draw_text(g, s_text, subFont, box.X + scaled(layout, 74.f), box.Y + scaled(layout, 39.f),
        Gdiplus::Color(255, 189, 189, 189));
}

void draw_vape(Gdiplus::Graphics& g, const Gdiplus::RectF& box,
    const notifications_layout::metrics& layout, float progress)
{
    const float cardRadius = scaled(layout, 6.f);
    const float barHeight = scaled(layout, 4.f);
    fill_card(g, box, Gdiplus::Color(252, 18, 18, 18), cardRadius,
        Gdiplus::Color(255, 34, 34, 34));

    Gdiplus::Font titleFont(L"Microsoft YaHei UI", scaled(layout, 14.5f), Gdiplus::FontStyleBold);
    Gdiplus::Font subFont(L"Segoe UI", scaled(layout, 9.5f), Gdiplus::FontStyleRegular);
    draw_text(g, s_text, titleFont, box.X + scaled(layout, 16.f), box.Y + scaled(layout, 11.f),
        Gdiplus::Color(255, 255, 255, 255));
    draw_text(g, s_enabledState ? L"Enable" : L"Disable", subFont,
        box.X + scaled(layout, 16.f), box.Y + scaled(layout, 36.f),
        Gdiplus::Color(255, 205, 205, 205));

    const Gdiplus::RectF track(box.X, box.Y + box.Height - barHeight, box.Width, barHeight);
...
void draw_gpt(Gdiplus::Graphics& g, const Gdiplus::RectF& box,
    const notifications_layout::metrics& layout, float progress)
{
    fill_card(g, box, Gdiplus::Color(235, 15, 18, 25), scaled(layout, 12.f),
        Gdiplus::Color(180, 90, 170, 255));
    Gdiplus::Font titleFont(L"Microsoft YaHei UI", scaled(layout, 13.f), Gdiplus::FontStyleBold);
    Gdiplus::Font subFont(L"Microsoft YaHei UI", scaled(layout, 10.f), Gdiplus::FontStyleRegular);
    draw_text(g, L"StrikeSense", titleFont, box.X + scaled(layout, 16.f), box.Y + scaled(layout, 11.f),
        Gdiplus::Color(255, 255, 255, 255));
    draw_text(g, s_text + (s_enabledState ? L" Enabled" : L" Disabled"), subFont,
        box.X + scaled(layout, 16.f), box.Y + scaled(layout, 36.f),
        Gdiplus::Color(230, 210, 220, 235));
    draw_progress(g, box, progress, layout,
        s_enabledState ? Gdiplus::Color(255, 74, 222, 150) : Gdiplus::Color(255, 255, 95, 95), false);
}

void draw_gemini(Gdiplus::Graphics& g, const Gdiplus::RectF& box,
    const notifications_layout::metrics& layout)
{
    fill_card(g, box, Gdiplus::Color(215, 0, 0, 0), scaled(layout, 10.f));
    Gdiplus::SolidBrush stripe(s_enabledState ? Gdiplus::Color(255, 80, 220, 120) : Gdiplus::Color(255, 230, 70, 70));
    g.FillRectangle(&stripe, Gdiplus::RectF(box.X, box.Y + scaled(layout, 8.f),
        scaled(layout, 3.f), box.Height - scaled(layout, 16.f)));
    Gdiplus::Font titleFont(L"Microsoft YaHei UI", scaled(layout, 13.f), Gdiplus::FontStyleBold);
    Gdiplus::Font subFont(L"Microsoft YaHei UI", scaled(layout, 10.f), Gdiplus::FontStyleRegular);
    draw_text(g, s_text, titleFont, box.X + scaled(layout, 16.f), box.Y + scaled(layout, 12.f),
        Gdiplus::Color(255, 255, 255, 255));
    draw_text(g, s_enabledState ? L"Enabled" : L"Disabled", subFont,
        box.X + scaled(layout, 16.f), box.Y + scaled(layout, 38.f),
        s_enabledState ? Gdiplus::Color(255, 150, 255, 190) : Gdiplus::Color(255, 255, 150, 150));
}

void draw_deepseek(Gdiplus::Graphics& g, const Gdiplus::RectF& box,
    const notifications_layout::metrics& layout)
{
    Gdiplus::RectF shadow(box.X + scaled(layout, 3.f), box.Y + scaled(layout, 5.f), box.Width, box.Height);
    fill_card(g, shadow, Gdiplus::Color(65, 0, 0, 0), scaled(layout, 12.f));
    fill_card(g, box, Gdiplus::Color(235, 14, 14, 20), scaled(layout, 12.f));

    Gdiplus::GraphicsPath path;
    add_rounded_rect(path, box, scaled(layout, 12.f));
    ...
    Gdiplus::Pen borderPen(&borderBrush, scaled(layout, 1.f));
    g.DrawPath(&borderPen, &path);

    Gdiplus::SolidBrush iconBack(Gdiplus::Color(45, 0, 242, 254));
    Gdiplus::SolidBrush iconBrush(Gdiplus::Color(255, 0, 242, 254));
    g.FillEllipse(&iconBack, box.X + scaled(layout, 18.f), box.Y + scaled(layout, 16.f),
        scaled(layout, 36.f), scaled(layout, 36.f));
    g.FillRectangle(&iconBrush, box.X + scaled(layout, 31.f), box.Y + scaled(layout, 25.f),
        scaled(layout, 10.f), scaled(layout, 18.f));
    g.FillRectangle(&iconBrush, box.X + scaled(layout, 25.f), box.Y + scaled(layout, 31.f),
        scaled(layout, 22.f), scaled(layout, 6.f));

    Gdiplus::Font titleFont(L"Microsoft YaHei UI", scaled(layout, 13.5f), Gdiplus::FontStyleBold);
    Gdiplus::Font subFont(L"Microsoft YaHei UI", scaled(layout, 9.f), Gdiplus::FontStyleRegular);
    draw_text(g, s_text, titleFont, box.X + scaled(layout, 68.f), box.Y + scaled(layout, 13.f),
        Gdiplus::Color(255, 255, 255, 255));
    draw_text(g, L"StrikeSense module toggle", subFont,
        box.X + scaled(layout, 68.f), box.Y + scaled(layout, 39.f),
        Gdiplus::Color(255, 161, 161, 170));
    ...
    Gdiplus::RectF pill(box.X + box.Width - scaled(layout, 116.f), box.Y + scaled(layout, 22.f),
        scaled(layout, 96.f), scaled(layout, 25.f));
    fill_card(g, pill, pillBg, scaled(layout, 12.f));
    Gdiplus::Font pillFont(L"Microsoft YaHei UI", scaled(layout, 9.f), Gdiplus::FontStyleBold);
    draw_text(g, state, pillFont, pill.X + scaled(layout, 17.f), pill.Y + scaled(layout, 5.f), pillText);
}
...
void draw()
{
    ...
    const notifications_layout::metrics layout = notifications_layout::CalculateMetrics(style, sw, sh);
    const float progress = std::clamp(1.f - elapsed / durationMs, 0.f, 1.f);
    const float liquidbounceKnobProgress = ease_out(elapsed / static_cast<float>(kLiquidBounceKnobAnimMs));
    const float in = style == 4 ? ease_bounce(elapsed / static_cast<float>(kAnimMs))
        : ease_out(elapsed / static_cast<float>(kAnimMs));
    const float outStart = (std::max)(0.f, durationMs - static_cast<float>(kAnimMs));
    const float out = elapsed > outStart
        ? ease_out((elapsed - outStart) / static_cast<float>(kAnimMs))
        : 0.f;
    ensure_timer(desired_frame_ms(elapsed, durationMs, style));

    int x = layout.baseX + static_cast<int>(std::lround((1.f - in + out) * layout.slideDistanceX));
    int y = layout.baseY;
    if (style == 1) {
        y = layout.baseY - static_cast<int>(std::lround((1.f - in + out) * layout.vapeSlideY));
    } else if (style == 4) {
        x = layout.baseX + static_cast<int>(std::lround(out * layout.slideDistanceX));
        y = layout.baseY - static_cast<int>(std::lround((1.f - in) * layout.deepseekSlideY));
    }

    HDC hdcScreen = GetDC(nullptr);
    if (!ensure_back_buffer(hdcScreen, layout.renderWidth, layout.renderHeight)) {
    ...
    const bool needsRedraw = s_cacheDirty
        ...
        || s_cachedWidth != layout.renderWidth
        || s_cachedHeight != layout.renderHeight;
    ...
    if (needsRedraw) {
        ...
        RectF box(static_cast<REAL>(layout.pad), static_cast<REAL>(layout.pad),
            static_cast<REAL>(layout.width), static_cast<REAL>(layout.height));

        if (style == 0) draw_liquidbounce(g, box, layout, liquidbounceKnobProgress);
        else if (style == 1) draw_vape(g, box, layout, progress);
        else if (style == 2) draw_gpt(g, box, layout, progress);
        else if (style == 3) draw_gemini(g, box, layout);
        else draw_deepseek(g, box, layout);
    ...
    POINT dst{ x - layout.pad, y - layout.pad };
    SIZE size{ layout.renderWidth, layout.renderHeight };
    POINT src{ 0, 0 };
    BLENDFUNCTION blend{};
...
void Initialize(HINSTANCE hInst)
{
    ...
    if (!s_hwnd) {
        std::cout << "[通知] 创建覆盖层失败。" << std::endl;
        return;
    }
    ShowWindow(s_hwnd, SW_HIDE);
    std::cout << "[通知] 覆盖层已创建。" << std::endl;
}
